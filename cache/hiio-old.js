...
async function payload (reqobj, mkbody) {
  ...
  if (bodytype === 'string') {
    ...
    let tmpbody = await mkbody.makeUploadData(opts.body)
    ...
    if (reqobj.headers['content-type'] === 'application/x-www-form-urlencoded') {
      formbody.data = Buffer.from(qs.stringify(opts.body))
    } else {
      formbody.data = Buffer.from(JSON.stringify(opts.body))
    }
...
class responsePool {
  ...
  constructor (max = 4096) {
    ...
    this.fillMax = parseInt(this.max * 0.8)
  ...
  free (r) {
    if (this.pool.length < this.max) {
      r.headers = r.data = r.timeout = r.ok = r.error = null
      r.buffers = []
      r.totalLength = r.status = r.contentLength = 0
      this.pool.push(r)
    }
  }
  ...
  fill () {
    ...
    if (this.pool.length < this.fillMax) {
      for (let i = 0; i < 50; i++) {
        this.pool.push(this.makeResponse())
      }
    }
...
class _Request {
  ...
  async request (reqobj, events = {}) {
    ...
    let rb = await payload(reqobj)

    let stm = this.session.request(reqobj.headers, reqobj.options || {})

    let ret = this.parent.respool.getResponse()
...
hiio.prototype._getPool = function (options, sr = null) {
  let r = this.pool.pop()
...
hiio.prototype._newRequest = function (options, sr = null) {
  if (sr) {
    return this._getPool(options, sr)
  }
